...
def wait_for(fd, stream, screen, needle, timeout=5):
    ...
    answered_dsr = False
    rendered = "\n".join(screen.display)
    if not hasattr(stream, "_beadsboard_decoder"):
        stream._beadsboard_decoder = codecs.getincrementaldecoder("utf-8")(
            errors="replace"
        )
    while time.monotonic() < deadline:
        ...
        if ready:
            ...
            matched = None
            for char in stream._beadsboard_decoder.decode(data):
                stream.feed(char)
                rendered = "\n".join(screen.display)
                if needle in rendered:
                    matched = rendered
            # Consume the full read, including trailing cursor/colour sequences,
            # before handing this stream to the next assertion.
            if matched is not None:
                return rendered if needle in rendered else matched
        rendered = "\n".join(screen.display)
        if needle in rendered:
            return rendered
    raise AssertionError(
        f"screen never contained {needle!r}:\n{rendered}\nraw={bytes(raw)!r}"
    )
...
def main():
    binary = pathlib.Path(
        sys.argv[1] if len(sys.argv) > 1 else "./beadsboard"
    ).resolve()
    with tempfile.TemporaryDirectory(prefix="beadsboard-pty-") as raw:
        ...
        record(
            agents / "agent-a.json", "agent-a", "epic-1", "terminal_7", sleepers[0].pid
        )
        record(
            agents / "agent-b.json", "agent-b", "epic-1", "terminal_8", sleepers[1].pid
        )
        ...
        if pid == 0:
            ...
            env.pop(
                "ZELLIJ", None
            )  # Enter reports which recorded pane it would reattach.
        ...
        try:
            ...
            while (agents / "agent-b.json").exists() and time.monotonic() < deadline:
                time.sleep(0.05)
            assert not (agents / "agent-b.json").exists(), (
                "k did not remove the focused registry record"
            )
            ...
            assert "session-agent-a" in focused, (
                "remaining agent was not focused for reattach"
            )
        finally:
            try:
